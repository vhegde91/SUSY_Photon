...
void myFittingDemo() {
  ...
  for(int i=1;i<=nBins;i++){
    data[i-1] = h_zmass->GetBinContent(i);
  }
  ...
  TH1F *histo = new TH1F("histo",
			 "Lorentzian Peak on Quadratic Background",500,0,500);
  ...
  TF1 *fitFcn = new TF1("fitFcn",fitFunction,0,500,6);
  fitFcn->SetNpx(5000);
  ...
  fitFcn->SetParameter(4,2); // width
  fitFcn->SetParameter(5,90);   // peak
  ...
  TF1 *signalFcn = new TF1("signalFcn",lorentzianPeak,0,500,3);
