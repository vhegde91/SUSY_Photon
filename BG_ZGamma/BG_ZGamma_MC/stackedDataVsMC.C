...
void stackedDataVsMC(TString iFname){
  ...
  f[1] = new TFile("CS_TTG_LLG_PtG190.root");
  f[2] = new TFile("CS_ZDYToLLG_LO_PtG190.root");
  ...
  name1.push_back("BestPhotonPt");   rebin.push_back(5); xLow.push_back(0); xHigh.push_back(1000);
  name1.push_back("ZMass");  rebin.push_back(2); xLow.push_back(70); xHigh.push_back(110);
  name1.push_back("METvarBin");  rebin.push_back(1); xLow.push_back(-100000); xHigh.push_back(1000000);
  ...
  for(int i=0;i<name1.size();i++){
    ...
    legend[i]=new TLegend(0.75, 0.88,  0.85, 0.55);
  ...
  for(int i=0;i<name1.size();i++){
    ...
    for(int p=1;p<nfiles;p++){  
      ...
      if(name1[i]=="METvarBin" && p==2){
	for(int iB=1;iB<=h_histE->GetNbinsX();iB++){
	  double ewErr=0;
	  if(h_histE->GetBinCenter(iB) >= 100 && h_histE->GetBinCenter(iB) < 200) ewErr=0.08;
	  else if(h_histE->GetBinCenter(iB) >= 200 && h_histE->GetBinCenter(iB) < 270) ewErr=0.18;
	  else if(h_histE->GetBinCenter(iB) >= 270 && h_histE->GetBinCenter(iB) < 350) ewErr=0.20;
	  else if(h_histE->GetBinCenter(iB) >= 350 && h_histE->GetBinCenter(iB) < 450) ewErr=0.25;
	  else if(h_histE->GetBinCenter(iB) >= 450 && h_histE->GetBinCenter(iB) < 750) ewErr=0.35;
	  else if(h_histE->GetBinCenter(iB) >= 750 ) ewErr=0.40;
	  h_histE->SetBinError(iB, sqrt(h_histE->GetBinError(iB)*h_histE->GetBinError(iB) + h_histE->GetBinContent(iB)*h_histE->GetBinContent(iB)*ewErr*ewErr));
	  //	  cout<<h_histE->GetBinContent(iB)<<" "<<h_histE->GetBinError(iB)<<" "<<100*h_histE->GetBinError(iB)/h_histE->GetBinContent(iB)<<endl;
	}
      }
...
TString getLegName(TString fname){
  ...
  else if(fname.Contains("CS_ZDYToLLG")) return "Z(ll)+#gamma";
  else if(fname.Contains("CS_TTG_LLG")) return "t#bar{t}+#gamma";
  else return fname;
}
TString getXaxisName(TString axname){
  ...
  else if(axname.Contains("ZMass")) return "m_{ll}(GeV)";
...
void setNegativeBinCnt0(TH1D* h_hist){
  for(int i=0;i<=h_hist->GetNbinsX();i++){
    if(h_hist->GetBinContent(i) < -0.0000001){
