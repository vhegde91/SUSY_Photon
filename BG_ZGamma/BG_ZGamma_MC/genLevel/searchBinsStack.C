...
void searchBinsStack(){
  ...
  f[7] = new TFile("ZGJetsToNuNuG.root");
  ...
  for(int i=0;i<nfiles;i++){
    ...
    TH1D *h_MET=(TH1D*)f[i]->FindObjectAny("MET_Rall");//MET_R1
    ...
    if(i==nfiles-1) hs_MET->SetTitle(";MET(GeV);Events");
  ...
  textOnTop.DrawLatexNDC(0.1,0.91,"CMS #it{#bf{Simulation}}");
  intLumiE.SetTextSize(0.06);
  sprintf(name2,"#bf{%0.2f fb^{-1}(13TeV)}",intLumi);
  intLumiE.DrawLatexNDC(0.73,0.91,name2);
  cout<<"*****************************************************"<<endl;
