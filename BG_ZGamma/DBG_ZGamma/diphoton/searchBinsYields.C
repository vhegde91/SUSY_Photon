...
void searchBinsYields(TString iFname){
  ...
  TFile *fMC = new TFile("CS_TTW_LostMuHadTau.root");
  ...
  TCanvas *c_cA=new TCanvas("allSearchBins_LostMuCS","CS yields in all search bins for lost muon",2000,900);
  ...
  TH1D *h_CSmc=new TH1D("searchBins_Mu1mc","mc searchBins 1 Mu:NJ[2,4][5,6],[7,inf)",21,0.5,21.5);
  TH1D *h_CSdata=new TH1D("searchBins_Mu1data","data searchBins 1 Mu:NJ[2,4][5,6],[7,inf)",21,0.5,21.5);
  ...
  // TFile *fout=new TFile("LostMu_SB.root","RECREATE");                                                                            
  // h_CSmc->Write();
  // h_CSdata->Write();
}
