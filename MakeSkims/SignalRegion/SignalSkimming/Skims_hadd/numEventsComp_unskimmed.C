...
void numEventsComp_unskimmed(){
  ...
  TString fName1,fName="root://cmseos.fnal.gov//store/user/lpcsusyhad/SusyPhotonMET/Run2ProductionV12/sorted_SMS/FastSim_GGM_M1M3_FastSim_GGM_M1M3_";
  TFile *f1;
  ...
  TFile *f2= TFile::Open("GGM_M1M3_MassScan.root");
  TH1D *h1;
  ...
  for(int i=1;i<=h2_mass->GetNbinsX();i++){
    for(int j=1;j<=h2_mass->GetNbinsY();j++){
      ...
      //      fName1=fName+to_string(mp)+"_"+to_string(md)+"_SkimmedSorted.root"; //for skimmed files
      fName1=fName+to_string(mp)+"_"+to_string(md)+".root";//for unskimmed files
      //      if(md < 100)       cout<<fName1<<endl;
      //      cout<<mp<<","<<md<<" ";
      f1=TFile::Open(fName1);
      if(!f1){ cout<<fName1<<" file open error"<<endl;continue;}
      tree = (TTree*)f1->Get("PreSelection");
      //      h1=(TH1D*)f1->FindObjectAny("selectBaselineYields_");
      double entries1;
      //      if(h1) entries1=h1->GetBinContent(1);
      if(tree) entries1=tree->GetEntries();
      totEvents1+=entries1;
