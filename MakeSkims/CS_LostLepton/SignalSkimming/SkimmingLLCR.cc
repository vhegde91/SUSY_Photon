#define SkimmingLLCR_cxx
#include "SkimmingLLCR.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <iostream>
...
int main(int argc, char* argv[])
{
  ...
  SkimmingLLCR ana(inputFileList, outFileName, data, s_mGl, s_mNLSP);
...
void SkimmingLLCR::EventLoop(const char *data,const char *inputFileList,const double mGl,const double mNLSP) {
  if (fChain == 0) return;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(Electrons->size()==1){
      if(sqrt(2*(*Electrons)[0].Pt()*MET*(1-cos(DeltaPhi(METPhi,(*Electrons)[0].Phi())))) > 100) continue;
    }
    if(Muons->size()==1){
      if(sqrt(2*(*Muons)[0].Pt()*MET*(1-cos(DeltaPhi(METPhi,(*Muons)[0].Phi())))) > 100) continue;
    }
    //---------------------------------------------------------------------------------
...
TLorentzVector SkimmingLLCR::getBestPhoton(){
  int bestPhoIndx=-100;
  TLorentzVector v1;
...
void SkimmingLLCR::storeBTagEff(){
