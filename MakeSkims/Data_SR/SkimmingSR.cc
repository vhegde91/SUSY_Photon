...
void SkimmingSR::EventLoop(const char *data,const char *inputFileList) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    //------------------ calc my clean variables --------------------
    TLorentzVector metCleanV;
    metCleanV.SetPtEtaPhiE(METclean,0.,METPhiclean,0.);
    TLorentzVector myCleanMET = metCleanV-bestPhoton;
    //---------------------------------------------------------------    
    ...
    if( MET > 100 || myCleanMET.Pt() > 100)                    h_selectBaselineYields_->Fill(5);
    else continue;
    //    cout<<"MET:"<<MET<<" ST:"<<ST<<" nHadJets:"<<nHadJets<<" bestPhoPt:"<<bestPhoton.Pt()<<" ne:"<<Electrons->size()<<" nmu:"<<Muons->size()<<endl;
