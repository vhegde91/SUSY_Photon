...
void compNNLOkFactors(){
  ...
  TFile *fn=new TFile("CS_ZGToNuNuG_NLO_PtG175_NNLOk.root");
  TFile *fd=new TFile("CS_ZGToNuNuG_LO_PtG175_NNLOk.root");

  TFile *fdUnc = new TFile("CS_ZGToNuNuG_LO_PtG175_NNLOk_ewCorr.root");
  ...
  TString nameN="NLO(Old method)";
  //TString nameN="Data";
  //  TString nameD="Z#gamma#rightarrow l^{+}l^{-}#gamma"; 
  TString nameD="LO(New method)"; 
  //TString nameD="MC";
  ...
  TH1D *h_num,*h_den,*h_numCp,*h_denUnc;
  for(int i=0;i<name2.size();i++){
	...
	ll_Integral=h_den->Integral();
      ...
      if(name2[i]=="AllSBins_v7"){
	TH1D *h_temp=(TH1D*)h_num->Clone("temp");
	for(int i=1;i<=h_num->GetNbinsX();i++){
	  h_temp->SetBinContent(i,0.);
	  if(i<=6) h_temp->SetBinError(i,0.14*h_num->GetBinContent(i));
	  else if(i>=17 && i<=21) h_temp->SetBinError(i,0.12*h_num->GetBinContent(i));
	  else h_temp->SetBinError(i,0.02*h_num->GetBinContent(i));
	}
	h_num->Add(h_temp);
	h_denUnc=(TH1D*)fdUnc->FindObjectAny(name2[i]);
	h_denUnc->Add(h_den,-1);
	for(int i=1;i<=h_den->GetNbinsX();i++){
	  h_denUnc->SetBinError(i,abs(h_denUnc->GetBinContent(i)));
	  h_denUnc->SetBinContent(i,0);
	}
	h_den->Add(h_denUnc);
      }
      //      setLastBinAsOverFlow(h_denUnc);
