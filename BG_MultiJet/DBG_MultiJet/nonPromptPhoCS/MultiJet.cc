...
void MultiJet::EventLoop(const char *data,const char *inputFileList) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    TLorentzVector bestPhoton=getBestPhoton(); if(bestPhoton.Pt()>100) continue;//veto SR events(blinded region)
    TLorentzVector idFailPhoton=getIDFailPhoton();
    bool isIDFailPho = false;
    if(bestPhoton.Pt() > 1.0) isIDFailPho = false;
    else if(bestPhoton.Pt() < 1.0 && idFailPhoton.Pt() > 1.0){
      bestPhoton = idFailPhoton;
      isIDFailPho = true;
      bestPhotonIndxAmongPhotons=idFailPhoIndx;
    }
    else continue;
    ...
    if(!useValRegion) continue;//use only VR. SR is blinded.    
    ...
    if( minDR<0.3 ) { photonMatchingJetIndx=minDRindx; nuemFracPhoJet = (*Jets_neutralEmEnergyFraction)[photonMatchingJetIndx];}
    for(int i=0;i<hadJets.size();i++){
      if( (abs(hadJets[i].Eta()) < HT_EtaCut) ){ST=ST+(hadJets[i].Pt());}
      if( (abs(hadJets[i].Eta()) < Njets_EtaCut) ){nHadJets++;}
    }
    //    bestPhoton = (*Jets)[photonMatchingJetIndx];
    if( minDR<0.3 ){
      myHT=ST;
      ST=ST+bestPhoton.Pt();
    }
...
TLorentzVector MultiJet::getIDFailPhoton(){
  vector<TLorentzVector> goodPho;
  vector<int> goodPhoIndx;
  // allBestPhotons.resize(0);
  for(int iPho=0;iPho<Photons->size();iPho++){
  ...
  if(highPtIndx>=0){
      idFailPhoIndx = goodPhoIndx[highPtIndx];
  }
  else idFailPhoIndx = -100;
