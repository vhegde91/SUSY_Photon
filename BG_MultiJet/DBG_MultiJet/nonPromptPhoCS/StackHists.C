...
void StackHists(){
  ...
  f[0] = new TFile("WGJetsToLNuG_idFail.root");
  f[1] = new TFile("TTGJets_idFail.root");
  f[2] = new TFile("TTJets_idFail.root");
  f[3] = new TFile("WJetsToLNu_idFail.root");
  f[4] = new TFile("ZJetsToNuNu_idFail.root");
  f[5] = new TFile("ZGJetsToNuNuG_idFail.root");
  f[6] = new TFile("QCD_idFail.root");
  f[7] = new TFile("GJets_idFail.root");
  ...
  cout<<left<<"[100,120)";cout.width(13);
  cout<<left<<"[120,160)";cout.width(13);
  cout<<left<<"[160,200)";cout.width(13);
  cout<<left<<"[200,270)";cout.width(13);
  cout<<left<<"[270,350)";cout.width(13);
  cout<<left<<"[350,450)";cout.width(13);
  cout<<left<<"[450,600)";cout.width(13);
  cout<<left<<"[600,750)";cout.width(13);
  cout<<left<<"[750,900)";cout.width(13);
  cout<<left<<"[900,Inf)";cout.width(13); cout<<endl;
  cout<<"====================================================================================================================================================================="<<endl;
  vector<double> Bcnt;
  double intLumi=0.0;
  TLatex tl1;
  for(int i=0;i<nfiles;i++){
    ...
    TH1D *h_ST=(TH1D*)f[i]->FindObjectAny("ST_CD");
    TH1D *h_MET=(TH1D*)f[i]->FindObjectAny("MET_CD");
    // TH1D *h_nJets=(TH1D*)f[i]->FindObjectAny("nJets");
    TH1D *h_nJets=(TH1D*)f[i]->FindObjectAny("nHadJets_CD");
    TH1D *h_BTags=(TH1D*)f[i]->FindObjectAny("nBTags_CD");
    TH1D *h_BestPhotonPt=(TH1D*)f[i]->FindObjectAny("BestPhotonPt_CD");
    //    TH2D *h2_PhotonPtvsMET=(TH2D*)f[i]->FindObjectAny("BestPhotonPtvsMET");
   
    // TH1D *h_STvBin=(TH1D*)f[i]->FindObjectAny("MET_Rall");
    // TH1D *h_METvBin=(TH1D*)f[i]->FindObjectAny("MET_R2");
    // TH1D *h_BestPhotonPtvBin=(TH1D*)f[i]->FindObjectAny("MET_R3");
    TH1D *h_STvBin=(TH1D*)f[i]->FindObjectAny("STvarBin_CD");
    TH1D *h_METvBin=(TH1D*)f[i]->FindObjectAny("METvarBin_CD");
    TH1D *h_BestPhotonPtvBin=(TH1D*)f[i]->FindObjectAny("BestPhotonPtvarBin_CD");
...
void drawlegend(TH1D *hist,int i,const char* fname){
  ...
  */
  ...
  if(lName=="ZJetsToNuNu_idFail"){lName="Z( #rightarrow #nu #bar{#nu})+jets";}
  else if(lName=="DYJetsToLL_idFail"){lName="DY( #rightarrow l^{+}l^{-})";}
  else if(lName=="WJetsToLNu_idFail"){lName="W( #rightarrow l  #nu)+jets";}
  else if(lName=="RareProcess_idFail"){}
  else if(lName=="TTJetsHT_idFail"){lName="t #bar{t}";}
  else if(lName=="QCD_idFail"){lName="QCD";}
  else if(lName=="GJets_idFail"){lName="#gamma +jets";}
  else if(lName=="WGJetsToLNuG_idFail"){lName="W(#rightarrow l #nu)+ #gamma";}
  else if(lName=="ZGJetsToNuNuG_idFail"){lName="Z(#rightarrow #nu #bar{#nu})+ #gamma";}
  else if(lName=="TTGJets_idFail"){lName="t #bar{t}+ #gamma";}
  else if(lName=="T1bbbb_ZG_mGl1600_NLSP150_idFail"){lName="4bZG_150";}
  else if(lName=="T1bbbb_ZG_mGl1600_NLSP1000_idFail"){lName="4bZG_1000";}
  else if(lName=="T1bbbb_ZG_mGl1600_NLSP1550_idFail"){lName="4bZG_1550";}
