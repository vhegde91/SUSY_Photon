...
double intLumi=35.9;
void compPhoSamples(TString iFname){
  TH1::SetDefaultSumw2(1);
  gStyle->SetOptStat(0);
  //  gStyle->SetOptStat("nemri"); 
  f[0] = new TFile(iFname);
  //  f[1] = new TFile("gjets_qcd.root");
  f[1] = new TFile("HLR_gjets_qcd.root");

  vector<string> name1,name2;
  vector<int> rebin;
  // name1.push_back("ST_AB");  rebin.push_back(5);         name2.push_back("ST_AB");     
  // name1.push_back("METvarBin_AB");    rebin.push_back(1);     name2.push_back("METvarBin_AB");
  // name1.push_back("nHadJets_AB");    rebin.push_back(1);     name2.push_back("nHadJets_AB");
  // name1.push_back("BestPhotonPt_AB"); rebin.push_back(2); name2.push_back("BestPhotonPt_AB");
  ...
  for(int i=0;i<name1.size();i++){
    ...
    h_numr->GetYaxis()->SetTitle("#frac{!SR #gamma}{SR #gamma}");
...
TString getLegName(TString fname){
  ...
  else if(fname.Contains("gjets_qcd")) return "SR Photon";
  else if(fname.Contains("GJetsQCD")) return "Not SR Photon";
