...
void MultiJet::EventLoop(const char *data,const char *inputFileList) {
  ...
  int decade = 0;
  bool do_AB_reweighting=0,useValRegion=1;
  int evtSurvived=0;
  ...
  //  if(useValRegion)  f_HLR = TFile::Open("HLR_VS_GJetsQCD_SR_Sel_v2.root");
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(bestPhoton.Pt()>1.0) continue;//if good photon is found, this is SR. So keep it blinded.
    ...
    ///Trigger related
    bool passTrigger = false;//(ST>800 && bestPhoton.Pt()>100);
    for(int i=0;i<TriggerNames->size();i++){
      TString trgName=(*TriggerNames)[i];
      if( trgName.Contains("HLT_CaloJet500_NoJetID_v") || trgName.Contains("HLT_PFHT800_v") || trgName.Contains("HLT_PFHT900_v")){
	if((*TriggerPass)[i]==1) passTrigger = true;
      }
    }
