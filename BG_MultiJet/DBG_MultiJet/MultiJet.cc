...
void MultiJet::EventLoop(const char *data,const char *inputFileList) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    //Trigger related
    bool passHT600Photon90Trigger = false;
    bool passPhoton165HE10Trigger = false;
    for(int i=0;i<TriggerNames->size();i++){
      TString trgName=(*TriggerNames)[i];
      if( trgName.Contains("HLT_Photon90_CaloIdL_PFHT600_v") && (*TriggerPass)[i]==1 ) passHT600Photon90Trigger = true;
      else if( trgName.Contains("HLT_Photon165_HE10_v") && (*TriggerPass)[i]==1 ) passPhoton165HE10Trigger = true;
    }
