...
void MultiJet::EventLoop(const char *data,const char *inputFileList) {
  ...
  int decade = 0;
  bool do_AB_reweighting=0,useValRegion=1,applISRWtsTottbar=1;
  int evtSurvived=0;
  TFile *f_HLR,*f_MCdoubleR,*f_reWtVR;
  TH1D *h_reWtVR;
  if(useValRegion){
    f_MCdoubleR = TFile::Open("HLR_VS_GJetsQCD_ST1000_v2.root");
    f_HLR = TFile::Open("HLR_EW_Subtracted_DVS_2016_ST1000.root");
  }
  // TFile *f_HLR;
  // //  if(useValRegion)  f_HLR = TFile::Open("HLR_VS_GJetsQCD_ST1000_v2.root");
  // if(useValRegion)  f_HLR = TFile::Open("HLR_VS_GJetsQCD_SR_Sele_v2.root");
  else  f_HLR = TFile::Open("HLR_GJetsQCD.root");
  //  TH1D *h_HLratio=(TH1D*)f_HLR->Get("HLratio_1D");
  TH2D *h2_HLRatio,*h2_MCdoubleR;
  ...
  cout<<"************* Apply ISR weights to ttbar? "<<applISRWtsTottbar<<endl;
  if(0){
    cout<<"!!!!!! Reweighting VR according to SR!!!!!"<<endl;
    f_reWtVR = TFile::Open("dPhiPhoMET_ratioOfSRtoVR.root");
  }
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    //    if((EvtNum%3)!=0) continue;
    //    if(MET>7000) continue;
    // if(Weight*lumiInfb*1000.0 > 10 && MET > 1000) continue;
    // if(Weight*lumiInfb*1000.0 > 1  && MET > 5000) continue;
    ///    if(Weight*lumiInfb*1000.0 > 5 && MET > 1000) continue;
    if(s_data!="FastSim"){
    ...
    int proxyJetIdx = -100;
    double proxyJetPhoFrac = 0.0,proxyJetMismeasure=100.;
    bool isProxyPho = false;
    if(useValRegion){
      for(int i=0;i<Jets->size();i++){
	if((*Jets)[i].Pt() < 100 || abs((*Jets)[i].Eta()) > 2.4 || ((*Jets_bDiscriminatorCSV)[i] > 0.8484) || (*Jets_photonEnergyFraction)[i] < 0.0) continue;
	//      	if((*Jets)[i].Pt() < 100 || abs((*Jets)[i].Eta()) > 2.4 || (*Jets_photonEnergyFraction)[i] < 0.0) continue;
      ...
      // //-------------------
      // // for(int i=0;i<GenJets->size() && proxyJetIdx >= 0;i++){
      // // 	if((*Jets)[proxyJetIdx].DeltaR((*GenJets)[i]) < 0.2)
      // // 	  proxyJetMismeasure = (abs((*Jets)[proxyJetIdx].Pt()-(*GenJets)[i].Pt())/(*GenJets)[i].Pt());
      // // }
      // // if(proxyJetMismeasure > 0.05) continue;//if proxy jet is mismeasured by > 10%, reject event
      // //-------------------
      // if((*Jets)[0].Pt() < 100 || abs((*Jets)[0].Eta()) > 2.4 || (*Jets_photonEnergyFraction)[0] < 0.6) continue;
      // proxyJetPhoFrac = (*Jets_photonEnergyFraction)[0];
      // proxyJetIdx = 0;
      //      proxyJetIdx = getBestJetIndx();
    ...
    if(s_data=="QCD"){
      ...
      if(s_data=="TTJets" && applISRWtsTottbar){
	double isrWt = 0;
    ...
    if( !((ST>800 && bestPhoton.Pt()>100) || (bestPhoton.Pt()>190)) ) continue;
    //    if(!(MET>450 && MET<750 && nHadJets>=2 && nHadJets<=4 && BTags==0)) continue;
    //    if(GenMET < 1 && Weight*lumiInfb*1000.0 > 1) continue;
    //    if(MET >750 && nHadJets>=2 && nHadJets<=4 && BTags==0 && GenMET < 1 && Weight*lumiInfb*1000.0 > 1) continue;
	...
	//------------------- block for GJets/QCD ----------------------------
        if(BTags==0) h2_MCdoubleR=(TH2D*)f_MCdoubleR->Get("HLRatio_0");
        else h2_MCdoubleR=(TH2D*)f_MCdoubleR->Get("HLRatio_1");
        double hlr1=h2_MCdoubleR->GetBinContent(h2_MCdoubleR->GetXaxis()->FindBin(105),h2_MCdoubleR->GetYaxis()->FindBin(parY));
        double doubleratio = (h2_MCdoubleR->GetBinContent(h2_MCdoubleR->GetXaxis()->FindBin(205),h2_MCdoubleR->GetYaxis()->FindBin(parY)))/hlr1;
        if(regType=='B') wt=wt*hlr1*doubleratio;
        else wt=wt*hlr1;
	//------------------- end of block for GJets/QCD ----------------------------
	//------------------- block for Weak processes. should be used for subtracting weak contribution in B  ----------------------------
        // if(BTags==0){
        //   h2_HLRatio=(TH2D*)f_HLR->Get("HLRatio_0");
        //   h2_MCdoubleR=(TH2D*)f_MCdoubleR->Get("HLRatio_0");
        // }
        // else{
        //   h2_HLRatio=(TH2D*)f_HLR->Get("HLRatio_1");
        //   h2_MCdoubleR=(TH2D*)f_MCdoubleR->Get("HLRatio_1");
        // }
        // double hlr1=h2_HLRatio->GetBinContent(h2_HLRatio->GetXaxis()->FindBin(105),h2_HLRatio->GetYaxis()->FindBin(parY));
        // double doubleratio = (h2_MCdoubleR->GetBinContent(h2_MCdoubleR->GetXaxis()->FindBin(205),h2_MCdoubleR->GetYaxis()->FindBin(parY))) / (h2_MCdoubleR->GetBinContent(h2_MCdoubleR->GetXaxis()->FindBin(105),h2_MCdoubleR->GetYaxis()->FindBin(parY)));
        // if(regType=='B') wt=wt*hlr1*doubleratio;
        // else wt=wt*hlr1;
	//------------------- end of block for Weak processes. should be used for subtracting weak contribution in B  ----------------------------
      }
      if(0){
	if(regType=='A' || regType=='B') h_reWtVR=(TH1D*)f_reWtVR->Get("AB_4J");
	if(regType=='C' || regType=='D') h_reWtVR=(TH1D*)f_reWtVR->Get("CD_4J");
	wt=wt*h_reWtVR->GetBinContent(h_reWtVR->FindBin(abs(dphiG_MET)));
      }
	...
	if(nHadJets==2) h_dPhi_METPhoton2J_[r_i]->Fill(abs(dphiG_MET),wt);
	else if(nHadJets==3) h_dPhi_METPhoton3J_[r_i]->Fill(abs(dphiG_MET),wt);
	else if(nHadJets==4) h_dPhi_METPhoton4J_[r_i]->Fill(abs(dphiG_MET),wt);
	else if(nHadJets==5 || nHadJets==6) h_dPhi_METPhoton5to6J_[r_i]->Fill(abs(dphiG_MET),wt);
	else h_dPhi_METPhoton7J_[r_i]->Fill(abs(dphiG_MET),wt);
	...
	h2_MET2vsWt_[r_i]->Fill(MET,Weight*lumiInfb*1000.0);
	h2_GenMET2vsWt_[r_i]->Fill(GenMET,Weight*lumiInfb*1000.0);
	...
	if(regType=='A' || regType=='B'){
	  ...
	  if(nHadJets==2) h_dPhi_METPhoton2J_AB->Fill(abs(dphiG_MET),wt);
	  else if(nHadJets==3) h_dPhi_METPhoton3J_AB->Fill(abs(dphiG_MET),wt);
	  else if(nHadJets==4) h_dPhi_METPhoton4J_AB->Fill(abs(dphiG_MET),wt);
	  else if(nHadJets==5 || nHadJets==6) h_dPhi_METPhoton5to6J_AB->Fill(abs(dphiG_MET),wt);
	  else h_dPhi_METPhoton7J_AB->Fill(abs(dphiG_MET),wt);
	  ...
	  h2_GenMET2vsWt_AB->Fill(GenMET,Weight*lumiInfb*1000.0);
	  h2_MET2vsWt_AB->Fill(MET,Weight*lumiInfb*1000.0);
	  h2_SBins_v4_vsEvtWt_AB->Fill(sBin4,Weight*lumiInfb*1000.0);
	  ...
	  if(BTags==0){
	    h2_METnHadJ_0b_AB->Fill(MET,nHadJets,wt);
	    h2_METnHadJ_0b_v2_AB->Fill(MET,nHadJets,wt);
	  }
          else{
	    h2_METnHadJ_min1b_AB->Fill(MET,nHadJets,wt);
	    h2_METnHadJ_min1b_v2_AB->Fill(MET,nHadJets,wt);
	  }
	...
	if(regType=='C' || regType=='D'){
	  // cout<<"BestPhoton:"<<bestPhoton.Pt()<<" "<<bestPhoton.Eta()<<" "<<bestPhoton.Phi()<<" "<<bestPhoton.M()<<endl;
	  // print(jentry);
	  // cout<<"BestPhoton:"<<bestPhoton.Pt()<<" "<<bestPhoton.Eta()<<" "<<bestPhoton.Phi()<<" "<<bestPhoton.M()<<endl;
	  // cout<<"dPhi1,2:"<<dphi1<<" "<<dphi2<<" dPhiPhoMET"<<dphiG_MET<<endl;
	  // print(jentry);
	  ...
	  if(nHadJets==2) h_dPhi_METPhoton2J_CD->Fill(abs(dphiG_MET),wt);
	  else if(nHadJets==3) h_dPhi_METPhoton3J_CD->Fill(abs(dphiG_MET),wt);
	  else if(nHadJets==4) h_dPhi_METPhoton4J_CD->Fill(abs(dphiG_MET),wt);
	  else if(nHadJets==5 || nHadJets==6) h_dPhi_METPhoton5to6J_CD->Fill(abs(dphiG_MET),wt);
	  else h_dPhi_METPhoton7J_CD->Fill(abs(dphiG_MET),wt);
	  ...
	  h2_GenMET2vsWt_CD->Fill(GenMET,Weight*lumiInfb*1000.0);
	  h2_MET2vsWt_CD->Fill(MET,Weight*lumiInfb*1000.0);
	  h2_SBins_v4_vsEvtWt_CD->Fill(sBin4,Weight*lumiInfb*1000.0);
	  ...
	  if(BTags==0){
	    h2_METnHadJ_0b_CD->Fill(MET,nHadJets,wt);
	    h2_METnHadJ_0b_v2_CD->Fill(MET,nHadJets,wt);
	  }
          else{
	    h2_METnHadJ_min1b_CD->Fill(MET,nHadJets,wt);
	    h2_METnHadJ_min1b_v2_CD->Fill(MET,nHadJets,wt);
	  }
...
int MultiJet::getBestJetIndx(){
  vector<double> misMeasr;
  vector<int> jetIdx;
  for(int i=0;i<Jets->size();i++){
    if((*Jets)[i].Pt() < 100 || (abs((*Jets)[i].Eta()) > 1.4442 && abs((*Jets)[i].Eta()) < 1.566) || abs((*Jets)[i].Eta()) > 2.4) continue;
    double minDR=100.;
    int minDRindx=-100;
    for(int j=0;j<GenJets->size();j++){
      if(minDR > (*Jets)[i].DeltaR((*GenJets)[j])){
	minDR = (*Jets)[i].DeltaR((*GenJets)[j]);
	minDRindx = j;
      }
    }
    if(minDRindx >=0){
      misMeasr.push_back(abs((*GenJets)[minDRindx].Pt()-(*Jets)[i].Pt())/(*GenJets)[minDRindx].Pt());
      jetIdx.push_back(i);
    }
  }//jets
  if(misMeasr.size()==0 || *min_element(misMeasr.begin(),misMeasr.end()) > 0.01) return -1;
  else return jetIdx[distance(misMeasr.begin(),min_element(misMeasr.begin(),misMeasr.end()))];
}
...
void MultiJet::print(Long64_t jentry){
  ...
  cout<<"MET:"<<MET<<" METPhi:"<<METPhi<<" GenMET:"<<GenMET<<" GenMETPhi:"<<GenMETPhi<<" HT:"<<HT<<" njets:"<<NJets<<" BTags:"<<BTags<<endl;
  for(int i=0;i<GenParticles->size();i++){
