...
void fitGauss(){
  ...
  TH1D *h = (TH1D*)f->Get("pull_multijet0Pho");
  //  h->Rebin(2);
  //  TF1 *ft = new TF1("ft","gaus",-2,3);
  TF1 *ft = new TF1("ft",fitfunc,-2,7,3);
  ft->SetParameters(6.,0.,.8);
  // ft->SetParLimits(0,7.5,14);
  // ft->SetParLimits(1,0,2);
  // ft->SetParLimits(2,0,8);
  ft->SetParLimits(0,7.5,8);
  ft->SetParLimits(1,0.2,2);
  ft->SetParLimits(2,0,3);
  h->Draw();
  h->Fit(ft,"R");
