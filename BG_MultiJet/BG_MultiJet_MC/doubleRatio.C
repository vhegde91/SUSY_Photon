...
int col[10]={kMagenta+2,kOrange,kBlack,kYellow,kGreen,kTeal+9,kPink+1,kCyan,kBlue,kRed};  //Specify Colors
TString name;
bool saveCanvas=0;
...
void doubleRatio(){
  ...
  //  f[1] = new TFile("gjets_qcd.root");
  f[0] = new TFile("HLR_CA_gjets_qcd.root");
  f[1] = new TFile("HLR_DB_gjets_qcd.root");

  vector<string> name1,name2;
  vector<int> rebin;
  name1.push_back("ST_C_ST_A");  rebin.push_back(1);         name2.push_back("ST_D_ST_B");     
  //  name1.push_back("METvarBin_AB");    rebin.push_back(1);     name2.push_back("METvarBin_AB");
  name1.push_back("nHadJets_C_nHadJets_A");    rebin.push_back(1);     name2.push_back("nHadJets_D_nHadJets_B");
  name1.push_back("nBTags_C_nBTags_A");    rebin.push_back(1);     name2.push_back("nBTags_D_nBTags_B");
  name1.push_back("BestPhotonPt_C_BestPhotonPt_A"); rebin.push_back(1); name2.push_back("BestPhotonPt_D_BestPhotonPt_B");
  name1.push_back("dPhi_METBestPhoton_C_dPhi_METBestPhoton_A");    rebin.push_back(1);     name2.push_back("dPhi_METBestPhoton_D_dPhi_METBestPhoton_B");
  // name1.push_back("dPhi_METBestPhoton_AB"); rebin.push_back(2); name2.push_back("dPhi_METBestPhoton_AB");

  // name1.push_back("ST_CD");  rebin.push_back(5);         name2.push_back("ST_CD");     
  // name1.push_back("METvarBin_CD");    rebin.push_back(1);     name2.push_back("METvarBin_CD");
  // name1.push_back("nHadJets_CD");    rebin.push_back(1);     name2.push_back("nHadJets_CD");
  // name1.push_back("BestPhotonPt_CD"); rebin.push_back(2); name2.push_back("BestPhotonPt_CD");
  // name1.push_back("dPhi_METBestPhoton_CD"); rebin.push_back(2); name2.push_back("dPhi_METBestPhoton_CD");
  //  name1.push_back("HLratio_1D");   name2.push_back("HLratio_1D");rebin.push_back(1); rebin.push_back(1);
  ...
  for(int i=0;i<name1.size();i++){
    ...
    legend[i]=new TLegend(0.80, 0.90,  0.90, 0.60);
  }
  ...
  for(int i=0;i<name1.size();i++){
    ...
    if(name1[i]=="nHadJets_AB") data_Integral=h_histG->Integral();
    ...
    //h_histG->Draw();
    legend[i]->AddEntry(h_histG,getLegName(f[0]->GetName()),"lep");
    //gPad->Update();
    // gPad->BuildLegend(0.75,0.75,0.95,0.95,"");
    h_histG->Draw();
    h_histG->SetTitle(";;H/L ratio");
    h_histG->GetYaxis()->SetTitleOffset(0.60);    
    h_histG->GetYaxis()->SetTitleSize(0.08);
    h_histG->GetYaxis()->SetLabelSize(0.08);
    for(int p=1;p<nfiles;p++){  
      ...
      // h_histE->SetTitle(";;Events");
      // h_histE->GetYaxis()->SetTitleOffset(0.60);    
    ...
    h_numr->GetYaxis()->SetTitle("#frac{C/A}{D/B}");
  ...
  // cout<<"Integral in !SR "<<data_Integral<<endl
  //     <<"Integral in SR "<<mc_Integral<<endl;
}

TString getLegName(TString fname){
  if(fname=="CS_TTG_LostMuHadTau_v2.root") return "t#bar{t}+#gamma";
  else if(fname.Contains("CA")) return "C/A";
  else if(fname.Contains("DB")) return "D/B";
  ...
  else if(fname=="FR_Hist_MS_FR_DYJetsToLL_v2.root") return "DY";
  else if(fname=="FR_Hist_CS_TTW_FR_NoTrg_v2.root") return "W/t#bar{t}";
  else return fname;
}
