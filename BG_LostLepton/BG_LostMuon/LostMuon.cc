...
void LostMuon::EventLoop(const char *data,const char *inputFileList) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    //    if(jentry>=31960)   cout<<"("<<fCurrent<<","<<jentry<<")";
    //    cout<<jentry<<" "<<Jets->size()<<" "<<JetsJECdown->size()<<" "<<METDown->size()<<" "<<METPhiDown->size()<<"|";
    //    cout<<jentry<<" ";
    //    if(jentry > 3207) print(jentry);
    //----------------- for jets ----------------
    Long64_t n_j = 0;
    ...
    if(MET < 100 || bestPhoton.Pt() < 100) continue;
