...
void searchBinsClosure(TString iFname){
  ...
  TCanvas *c_cA=new TCanvas("allSearchBins_LostMuClsr","all search bins closure for lost muon",1500,800);
  ...
  //  h_numr->SetTitle(";;#frac{0#mu+#gamma}{1#mu+#gamma}");
  ...
  TLegend *legend=new TLegend(0.65, 0.90,  0.90, 0.7);
  legend->AddEntry(h_SR,"0 #mu+Had#tau+#gamma (Exp.)","l");
  //  legend->AddEntry(h_SR,"0 #mu+#gamma (Exp.)","l");
  //legend->AddEntry(h_SR,"Had#tau+#gamma (Exp.)","l");
  legend->AddEntry(h_CR,"1 #mu+#gamma (Pred.)","l");
  legend->Draw();
  ...
  // TFile *fout=new TFile("LostMuon_SB.root","RECREATE");
  // h_CR->Write();
  // h_SR->Write();
