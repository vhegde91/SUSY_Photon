...
// void SetRatioErr(TH1D*TotalBkg, TH1D*Pred, TGraphAsymmErrors &Closure,
                 //int nBins = 4 , double lowerBound = 100. , double upperBound = 900.,
//                 bool verbose = false){
void computeRatio(TString fName,
                 //int nBins = 4 , double lowerBound = 100. , double upperBound = 900.,
                 bool verbose = false){

  TFile *f=new TFile(fName);
  TH1D *TotalBkg=(TH1D*)f->Get("AllSBins_v7_Mu0");
  TH1D *Pred=(TH1D*)f->Get("AllSBins_v7_Mu1");
  TGraphAsymmErrors *Closure = new TGraphAsymmErrors();
    ...
    TH1D*hratio=new TH1D("hratio", "", nBins,bins);
	...
	//        if(m==4) Closure->SetPointError(m-1, (bins[m]-bins[m-1])/2.,(bins[m]-bins[m-1])/2., RatioErrorDn,RatioErrorUp);
        if( verbose )
	  std::cout<<m<<" Mean: "<<METBinKappa->GetBinContent(m)<<" +/- "<< RatioErrorUp<<" Asym:"<<RatioErrorUp-RatioErrorDn<<std::endl;
    }
    kappaDist->Draw();
    //    Closure->Draw("AP");
}
