...
void numEventsComp(){
  ...
  TString fName1,fName="root://cmseos.fnal.gov//store/user/vhegde/GMSB_skims_ST_RA2b_TreesV12/signalSystematics/hists_t5qqqqHg/FastSim_T5qqqqHg_SystA_";
  TFile *f1;
  ...
  for(int i=1;i<=h2_mass->GetNbinsX();i++){
    for(int j=1;j<=h2_mass->GetNbinsY();j++){
      ...
      fName1=fName+to_string(mp)+"_"+to_string(md)+".root";
      //      cout<<fName1<<endl;
      //      cout<<mp<<","<<md<<" ";
      f1=TFile::Open(fName1);
      if(!f1){ cout<<fName1<<" file open error"<<endl;continue;}
      h1=(TH1D*)f1->FindObjectAny("cutFlow");
      double entries1;
...
//bool numEventsComp(TString inFile,double mp,double md){
  ...
  //  TFile *f2= TFile::Open("T5bbbbZg_MassScan.root");
  TFile *f2= TFile::Open("T5qqqqHg_MassScan.root");
  TH1D *h1=(TH1D*)f1->FindObjectAny("cutFlow");
  TH2D *h2_mass = (TH2D*)f2->FindObjectAny("MGlMNLSP");
  ...
  if(abs(entries2-entries1) < 0.1){
    cout<<"yyyy"<<endl;
    //    sprintf(cmd1,"eos root://cmseos.fnal.gov rm /store/user/vhegde/GMSB_skims_ST_RA2b_TreesV12/SignalRegion_v1/temp/T5qqqqHg_%.0f_%.0f_*pythia8_job*.root",mp,md);
    //    cout<<cmd1<<endl;
