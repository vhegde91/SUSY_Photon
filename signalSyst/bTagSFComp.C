...
void bTagSFComp(TString iFname0, TString iFname1){
  TH1::SetDefaultSumw2(1);
  gStyle->SetOptStat(0);
 
  f[0] = new TFile(iFname0);
  f[1] = new TFile(iFname1);

  TString outFileName = "bTagSF_T5bbbbZg_1600_150";
  ...
  TLine *line1=new TLine( 8.5,0.05,  8.5,10);
  TLine *line2=new TLine(15.5,0.05, 15.5,10);
  TLine *line3=new TLine(22.5,0.05, 22.5,10);
  TLine *line4=new TLine(29.5,0.05, 29.5,10);
  TLine *line5=new TLine(36.5,0.05, 36.5,1 );
  ...
  for(int i=0;i<name1.size();i++){
	...
	cr_Integral=h_histE->Integral();
      ...
      name=name1[i];
      legend[i]->AddEntry(h_histG,getLegName(f[0]->GetName()),"lp");
      name=name2[i];
      legend[i]->AddEntry(h_histE,getLegName(f[1]->GetName()),"lp");
      legend[i]->Draw();
  ...
  //h_numr->SetTitle(";;#frac{Exp}{Pred }");
    ...
    h_numr->GetYaxis()->SetTitle("#frac{"+getLegName(f[0]->GetName())+"}{"+getLegName(f[1]->GetName())+"}");
    h_numr->SetMaximum(1.99);
    h_numr->SetMinimum(0.001);
    c_cA[i]->cd();    p_bot[i]->cd();	p_bot[i]->SetGridx(0);
...
TString getLegName(TString legName1){
  if(legName1.Contains("bTagUnc")) return "SF up";
