...
void LostMuon::EventLoop(const char *data,const char *inputFileList) {
 ...
 //-----------ISR rewighting for signal ------------------
  ...
  TFile *fISR;  TH1D *h_xsecGl; TH2D *h2_numEvts, *h2_isrWtCorr, *h2_xsecGl;
  ...
  if(s_data.Contains("FastSim")){
    ...
    else if(s_data.Contains("GGM_M1M3")){fISR = new TFile("GGM_M1M3_MassScan.root"); h2_xsecGl = (TH2D*)fISR->Get("GGM_M1M3_xsec");}//for GGM
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	CrossSection = h2_xsecGl->GetBinContent(h2_xsecGl->GetXaxis()->FindBin(SusyMotherMass),h2_xsecGl->GetYaxis()->FindBin(SusyLSPMass));
      else
    ...
    if(process && hadJetID){
      //      cout<<"SUSY mom:"<<SusyMotherMass<<" Kid:"<<SusyLSPMass<<" xsec:"<<setprecision(9)<<CrossSection<<" NumEvts:"<<NumEvents<<" Wt:"<<setprecision(9)<<Weight<<" myWt:"<<setprecision(9)<<wt<<" ISR wt:"<<isrWt<<" ISRwtCorrc:"<<isrWtCorr<<endl;
      evtSurvived++;
      double wt_org=wt;
