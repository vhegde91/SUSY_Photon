...
void FakeRateEst::EventLoop(const char *data,const char *inputFileList) {
  ...
  bool applISRWtsTottbar=0,applWPtWts=1;
  ...
  cout<<"!!!!!!!!!!!!!!!!!!!!!!!!"<<endl;
  cout<<"Electrons are replaced by muons."<<endl;
  cout<<"!!!!!!!!!!!!!!!!!!!!!!!!"<<endl;
  //---------------------- MC only -------------------------
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    wt=Weight*lumiInfb*1000.0*0.88*(puhist->GetBinContent(puhist->GetXaxis()->FindBin(min(TrueNumInteractions,puhist->GetBinLowEdge(puhist->GetNbinsX()+1)))));  
    //    wt=Weight*1000.0*lumiInfb;  
    if(s_data=="madHT0to600" && madHT > 600) continue;
    if(applISRWtsTottbar){
      double isrWt = 0;
