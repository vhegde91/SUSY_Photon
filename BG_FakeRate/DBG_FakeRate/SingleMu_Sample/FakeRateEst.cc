...
void FakeRateEst::EventLoop(const char *data,const char *inputFileList) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if( Muons->size()>1 || Electrons->size()!=0 ) continue;
    if(isoElectronTracks!=0 || isoPionTracks!=0) continue;   
    //about electrons
    bool hasEle=0,hasPho=0;
    if(Muons->size()==1){
      if( ((*Muons)[0].Pt()>100) && (*Muons_tightID)[0] ) hasEle=true;
    }
    //about photons
    ...
    if     ( hasEle && !hasPho) {bestEMObjIsEle=true;  bestEMObj = (*Muons)[0];}
    ...
    for(int i=0;i<TriggerNames->size();i++){
      ...
      if( trgName.Contains("HLT_Mu50_v") && (*TriggerPass)[i]==1 ) passTrig = true;
      wt=1.0;
      //      wt=0.98/0.88;
      // if((*Muons)[0].Pt() > 190.0){
    ...
    if(process){
      ...
      //      cout<<" "<<Muons_MT2Activity->size()<<","<<Muons->size();      
      int searchRegion=0;
	...
	//   if(min_1>bestEMObj.DeltaR((*GenMuons)[i])){
	//     min_1=bestEMObj.DeltaR((*GenMuons)[i]);
	//     genEleMT2Act=(*GenMuons_MT2Activity)[i];
	//   }
	// }
	...
	if(bestEMObjIsEle){ //+++++ Data only +++++
	  ...
	  h_EleTracks_Ele->Fill(TAPMuonTracks->size(),wt);
	  h_MuTracks_Ele->Fill(TAPMuonTracks->size(),wt);
	  h_PiTracks_Ele->Fill(TAPPionTracks->size(),wt);
	  h_nTracks_Ele->Fill(TAPMuonTracks->size()+TAPMuonTracks->size()+TAPPionTracks->size(),wt);
	  ...
	  for(int i=0;i<Muons->size();i++){
	    if(tempMin>bestEMObj.DeltaR((*Muons)[i])){
	      tempMin=bestEMObj.DeltaR((*Muons)[i]);
	      eleMT2Act=(*Muons_MT2Activity)[i];
	    }
	  }
	...
	//+++++++++++++++++++++++++ data only +++++++++++++++++++++++++++++
	  ...
	  h_EleTracks_Pho->Fill(TAPMuonTracks->size(),wt);
	  h_MuTracks_Pho->Fill(TAPMuonTracks->size(),wt);
	  h_PiTracks_Pho->Fill(TAPPionTracks->size(),wt);
	  h_nTracks_Pho->Fill(TAPMuonTracks->size()+TAPMuonTracks->size()+TAPPionTracks->size(),wt);
