...
void testClosure(TString iFname){
  ...
  for(int i=0;i<name1.size();i++){
	...
	for(int k=1;k<=h_histG->GetNbinsX();k++){
	  if(name1[i]=="ST_Mu0")  cout<<h_histG->GetBinContent(k)<<" +/- "<<h_histG->GetBinError(k)<<"\t"<<h_histG->GetBinLowEdge(k)<<endl;
	}cout<<endl;
	for(int k=1;k<=h_histE->GetNbinsX();k++){
	  if(name1[i]=="ST_Mu0")  cout<<h_histE->GetBinContent(k)<<" +/- "<<h_histE->GetBinError(k)<<"\t"<<h_histE->GetBinLowEdge(k)<<endl;
	}cout<<endl;
      }
    }
    ...
    for(int k=1;k<=h_numr->GetNbinsX();k++){
      if(name1[i]=="ST_Mu0")  cout<<h_numr->GetBinContent(k)<<" +/- "<<h_numr->GetBinError(k)<<"\t"<<h_numr->GetBinLowEdge(k)<<endl;
    }cout<<endl;
