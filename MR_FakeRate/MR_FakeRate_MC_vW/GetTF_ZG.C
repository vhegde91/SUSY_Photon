...
void GetTF_ZG(){
  ...
  name2.push_back("ZMass");  rebin.push_back(1);
  ...
  TString nameN="Data";
  TString nameD="Z#gamma#rightarrow l^{+}l^{-}#gamma(MC)"; 
  ...
  for(int i=0;i<name2.size();i++){
    ...
    c_cA[i] = new TCanvas(name,name,1500,850);    
    p_top[i] =new TPad(name+"top",name+"top",0,0.35,1,1);
    p_bot[i] =new TPad(name+"bot",name+"bot",0,0.04,1,0.35);
  ...
  for(int i=0;i<name2.size();i++){
	...
	ll_Integral=h_den->Integral();
      ...
      legend[i]=new TLegend(0.75, 0.90,  0.90, 0.7);
      ...
      h_numCp->GetXaxis()->SetLabelSize(0.13);
      h_numCp->GetXaxis()->SetTitleSize(0.12);
      ...
      h_numCp->GetYaxis()->SetTitleOffset(0.29);
