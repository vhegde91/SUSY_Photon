...
void findFailedJobs(){
  string jdlStart="/eos/uscms/store/user/vhegde/GMSB_skims_ST_RA2b_TreesV12/MS_EleFakes/tnpTrees/";
  findFailedJobs(jdlStart);
}
void findFailedJobs(string arg){
  char jdlStart[200];
  string FileStart="/eos/uscms/store/user/vhegde/GMSB_skims_ST_RA2b_TreesV12/MS_EleFakes/tnpTrees/"+arg;
  ///eos/uscms/store/user/vhegde/GMSB_skims_ST_RA2b_TreesV12/MS_EleFakes/tnpTrees/runList_MS_FR_TTGJets_job0.root
  sprintf(jdlStart,"%s",FileStart.c_str());
  //  sprintf(jdlStart,"signalRegionSkim_%s_",dataset);
  char ofileStart[300]=" ";
  char name1[200],name2[400],name3[300];
  gEnv->SetValue("TFile.Recover", 0);
  vector<int> failedID;
  int totalJobs=0;
  for(int i=0;;i++){
    sprintf(name1,"%s_job%i.jdl",jdlStart,i);//cout<<name1<<endl;
    sprintf(name3,"%s_job%i.jdl",arg.c_str(),i);
    ifstream infile(name3);
