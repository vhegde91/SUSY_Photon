...
class MeasureFR : public NtupleVariables{
 ...
 public:
  ...
  TH1D *h_minDR_ElerL1_Ele;
  TH1D *h_minDR_ElefL1_Ele;
  TH1D *h_minDR_ElerHLT_Ele;
  TH1D *h_minDR_ElefHLT_Ele;
  ...
  TH2D *h2_ElefPt_ZMass_Ele;
  TH2D *h2_ElefPtvBin_ZMass_Ele;
  TH2D *h2_QMultJetElef_ZMass_Ele;
  TH2D *h2_ST_ZMass_Ele;
  TH2D *h2_nHadJets_ZMass_Ele;
  //--------------------- ge(fake) ----------------
  TH1D *h_1Bin_Pho;
  TH1D *h_ST_Pho;
  TH1D *h_MET_Pho;
  ...
  TH1D *h_minDR_ElerL1_Pho;
  TH1D *h_minDR_ElefL1_Pho;
  TH1D *h_minDR_ElerHLT_Pho;
  TH1D *h_minDR_ElefHLT_Pho;
  ...
  TH2D *h2_ElefPt_ZMass_Pho;
  TH2D *h2_ElefPtvBin_ZMass_Pho;
  TH2D *h2_QMultJetElef_ZMass_Pho;
  TH2D *h2_ST_ZMass_Pho;
  TH2D *h2_nHadJets_ZMass_Pho;
  //---------------- search bins -----------------------                                                                        
  TH1D *h_GmatchedObj;
  ...
  TTree *tnpTree;
  bool pho_=true;
  int isbg_=-1,nhadjets_=0,qmultprobe_=-1,qmulttag_=-1,runnum_=0,evtnum_=0;
  double st_=0,met_=0,tagpt_=0,probept_=0,tageta_=5.8,tagphi_=3.8,probeeta_=5.8,probephi_=3.8,zmass_=0,zmasspho_=0,zmassele_=0,wt_=0;
...
void MeasureFR::BookHistogram(const char *outFileName) {
  ...
  h_passTrig=new TH1D("passTrig","Passes Ele27WPTight trigger",5,-2,3);

  tnpTree=new TTree("tnpTree","tree for e faking photon measurement"); 
  tnpTree->Branch("RunNum",&runnum_,"runnum_/I");
  tnpTree->Branch("EvtNum",&evtnum_,"evtnum_/I");
  tnpTree->Branch("ST",&st_,"st_/D");
  tnpTree->Branch("ProbeIsPhoton",&pho_,"pho_/B");
  tnpTree->Branch("IsBGEvent",&isbg_,"isbg_/I");
  tnpTree->Branch("MET",&met_,"met_/D");
  tnpTree->Branch("HadJets",&nhadjets_,"nhadjets_/I");
  tnpTree->Branch("ProbePt",&probept_,"probept_/D");
  tnpTree->Branch("TagPt",&tagpt_,"tagpt_/D");
  tnpTree->Branch("ProbeEta",&probeeta_,"probeeta_/D");
  tnpTree->Branch("TagEta",&tageta_,"tageta_/D");
  tnpTree->Branch("ProbePhi",&probephi_,"probephi_/D");
  tnpTree->Branch("TagPhi",&tagphi_,"tagphi_/D");
  tnpTree->Branch("QMultProbeJet",&qmultprobe_,"qmultprobe_/I");
  tnpTree->Branch("QMultTagJet",&qmulttag_,"qmulttag_/I");
  tnpTree->Branch("ZMass",&zmass_,"zmass_/D");
  tnpTree->Branch("ZMass_Pho",&zmasspho_,"zmasspho_/D");
  tnpTree->Branch("ZMass_Ele",&zmassele_,"zmassele_/D");
  tnpTree->Branch("EvtWt",&wt_,"wt_/D"); 
  ...
  h_nHadJets_Ele=new TH1D("nHadJets_Ele","nHadJets for ee events",25,-0.5,24.5);
  ...
  h_minDR_ElerL1_Ele=new TH1D("minDR_ElerL1_Ele","min dR(Tag e,L1 obj)",300,0,3);
  h_minDR_ElefL1_Ele=new TH1D("minDR_ElefL1_Ele","min dR(probe e,L1 obj)",300,0,3);
  h_minDR_ElerHLT_Ele=new TH1D("minDR_ElerHLT_Ele","min dR(Tag e,HLT obj)",300,0,3);
  h_minDR_ElefHLT_Ele=new TH1D("minDR_ElefHLT_Ele","min dR(probe e,HLT obj)",300,0,3);
  ...
  h2_ElefPt_ZMass_Ele=new TH2D("ElefPt_ZMass_Ele","x: F_ElePt vs Z mass for ee events",150,0,1500,8,0,160);
  h2_ElefPtvBin_ZMass_Ele=new TH2D("ElefPtvBin_ZMass_Ele","x: Electron_f Pt var bins vs Z mass for ee events",ptLow4.size()-1,&(ptLow4[0]),8,0,160);
  h2_QMultJetElef_ZMass_Ele=new TH2D("QMultJetElef_ZMass_Ele","x: Q mult in jet closest to e_f vs Z mass for ee events",50,0,50,8,0,160);
  h2_ST_ZMass_Ele=new TH2D("ST_ZMass_Ele","x: ST vs Z Mass for ee events",400,0,4000,8,0,160);
  h2_nHadJets_ZMass_Ele=new TH2D("nHadJets_ZMass_Ele","x: nHad Jets vs Z Mass for ee events",25,-0.5,24.5,8,0,160);
  //---------------------------- ge ------------------------------------
  ...
  h_nHadJets_Pho=new TH1D("nHadJets_Pho","nHadJets for e(->#gamma)e events",25,-0.5,24.5);
  ...
  h_minDR_ElerL1_Pho=new TH1D("minDR_ElerL1_Pho","min dR(Tag e,L1 obj)",300,0,3);
  h_minDR_ElefL1_Pho=new TH1D("minDR_ElefL1_Pho","min dR(probe #gamma,L1 obj)",300,0,3);
  h_minDR_ElerHLT_Pho=new TH1D("minDR_ElerHLT_Pho","min dR(Tag e,HLT obj)",300,0,3);
  h_minDR_ElefHLT_Pho=new TH1D("minDR_ElefHLT_Pho","min dR(probe #gamma,HLT obj)",300,0,3);
  ...
  h2_ElePtEtavBin=new TH2D("ElePtEtavBin","x:Electron |Eta| vs Electron Pt for e(->#gamma)e events",5,etaLow,7,ptLow2);
  h2_ElefPt_ZMass_Pho=new TH2D("ElefPt_ZMass_Pho","x: F_ElePt vs Z mass for e(->#gamma)e events",150,0,1500,8,0,160);
  h2_ElefPtvBin_ZMass_Pho=new TH2D("ElefPtvBin_ZMass_Pho","x: Electron_f Pt var bins vs Z mass for e(->#gamma)e events",ptLow4.size()-1,&(ptLow4[0]),8,0,160);
  h2_QMultJetElef_ZMass_Pho=new TH2D("QMultJetElef_ZMass_Pho","x: Q mult in jet closest to e_f vs Z mass for e(->#gamma)e events",50,0,50,8,0,160);
  h2_ST_ZMass_Pho=new TH2D("ST_ZMass_Pho","x: ST vs Z Mass for e(->#gamma)e events",400,0,4000,8,0,160);
  h2_nHadJets_ZMass_Pho=new TH2D("nHadJets_ZMass_Pho","x: nHad Jets vs Z Mass for e(->#gamma)e events",25,-0.5,24.5,8,0,160);
  //********************************************************
