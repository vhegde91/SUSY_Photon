...
*/
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    double mindr1L1=100,mindr2L1=100,mindr1HLT=100,mindr2HLT=100;
    vector<bool> l1MatchEle, hltMatchEle;
    for(int i=0;i<Electrons->size();i++){
      bool l1match=false,hltmatch=false;
      for(int j=0;j<L1EGObjects->size();j++){
	if((*Electrons)[i].DeltaR((*L1EGObjects)[j]) < 0.2){
	  l1match = true; //break;
	}
      }
    ...
    bool temp1=false;
    for(int i=0;i<Electrons->size();i++){
      if(hltMatchEle[i]) temp1=true;
    }
    //    if(!temp1) print(jentry);
    //++++++++++++++++++++++++++++++++++++++++++++++++++++++++    
    //about photons
    bool hasPho = false;
	...
	if( ((*Electrons)[0].DeltaR((*TAPElectronTracks)[i]) > 0.2)){
      ...
      if(!(hltMatchEle[0])) continue;//match e to HLT
      eler = (*Electrons)[0];
	...
	if(hltMatchEle[0] && !hltMatchEle[1]){//only leading e matched to HLT
	  eler = (*Electrons)[0];
	  elef = (*Electrons)[1];
	}
	else if(!hltMatchEle[0] && hltMatchEle[1]){//only sub-leading e is matched to HLT
	  eler = (*Electrons)[1];
	  elef = (*Electrons)[0];
	}
	else if(hltMatchEle[0] && hltMatchEle[1]){//both e are mathced to HLT
	...
	else cout<<"AHHHHHH:None of the electrons matched to HLT. Expect at least one 1 should have mathced.";
      }
      else cout<<"AHHHHH: logic issue in choosing tag and probe from ee"<<endl;
      // if( !((*Electrons_tightID)[0]) && !((*Electrons_tightID)[1]) ) continue;//LL ee
...
*/
    ...
    if(process && hadJetID){
      ...
      tageta_=eler.Eta();
      tagphi_=eler.Phi();
      qmulttag_=qMultJetElef;
 
      probept_=elef.Pt();
      probeeta_=elef.Eta();
      probephi_=elef.Phi();
      qmultprobe_=qMultJetEler;
 
      zmass_=zmass;
      if(hasPho){ zmasspho_=zmass; zmassele_=0;}
      if(!hasPho){ zmassele_=zmass; zmasspho_=0;}
	...
	if(mindr1HLT > (*HLTElectronObjects)[i].DeltaR(eler)) mindr1HLT = (*HLTElectronObjects)[i].DeltaR(eler);
	if(mindr2HLT > (*HLTElectronObjects)[i].DeltaR(elef)) mindr2HLT = (*HLTElectronObjects)[i].DeltaR(elef);
      }
      // h_minDR_ElerL1_Ele->Fill(mindr1L1,wt);
      // h_minDR_ElefL1_Ele->Fill(mindr2L1,wt);
	...
	h_minDR_ElerHLT_Ele->Fill(mindr1HLT,wt);
	h_minDR_ElefHLT_Ele->Fill(mindr2HLT,wt);
	...
	h2_ElefPt_ZMass_Ele->Fill(elef.Pt(),zmass,wt);
	h2_ElefPtvBin_ZMass_Ele->Fill(elef.Pt(),zmass,wt);
	h2_QMultJetElef_ZMass_Ele->Fill(qMultJetElef,zmass,wt);
	h2_ST_ZMass_Ele->Fill(ST,zmass,wt);
	h2_nHadJets_ZMass_Ele->Fill(nHadJets,zmass,wt);
	...
	h_minDR_ElerHLT_Pho->Fill(mindr1HLT,wt);
	h_minDR_ElefHLT_Pho->Fill(mindr2HLT,wt);
	...
	h2_ElefPt_ZMass_Pho->Fill(elef.Pt(),zmass,wt);
	h2_ElefPtvBin_ZMass_Pho->Fill(elef.Pt(),zmass,wt);
	h2_QMultJetElef_ZMass_Pho->Fill(qMultJetElef,zmass,wt);
	h2_ST_ZMass_Pho->Fill(ST,zmass,wt);
	h2_nHadJets_ZMass_Pho->Fill(nHadJets,zmass,wt);
	//---------------- search bins -----------------------
	if( searchRegion > 0 && searchRegion < 4){
...
*/  
  ...
  for(int i=0;i<L1EGObjects->size();i++){
    cout<<"L1 Pt: "<<(*L1EGObjects)[i].Pt()<<" eta: "<<(*L1EGObjects)[i].Eta()<<" phi: "<<(*L1EGObjects)[i].Phi()<<" E: "<<(*L1EGObjects)[i].E()<<endl;
  }
  for(int i=0;i<HLTElectronObjects->size();i++){
    cout<<"HLT Pt: "<<(*HLTElectronObjects)[i].Pt()<<" eta: "<<(*HLTElectronObjects)[i].Eta()<<" phi: "<<(*HLTElectronObjects)[i].Phi()<<" E: "<<(*HLTElectronObjects)[i].E()<<endl;
  }
