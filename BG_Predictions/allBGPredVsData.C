...
void allBGPredVsData(TString iFname){
  ...
  f[0] = new TFile(iFname);                                      suffixHist.push_back("_CD");
  f[1] = new TFile("CS_ZGZJToNuNuG.root");                       suffixHist.push_back("");
  f[2] = new TFile("DCS_Run2016_LostEle_v2.root");               suffixHist.push_back("_Ele0");
  f[3] = new TFile("DCS_Run2016_LostMuHadTau_v2.root");          suffixHist.push_back("_Mu0");
  f[4] = new TFile("DCS_Run2016_SingleEle_v2.root");             suffixHist.push_back("_Pho");
  f[5] = new TFile("DCS_Run2016_Multijet_Pred_v2.root");         suffixHist.push_back("_CD");
  ...
  // f[5] = new TFile("DCS_LDP_Run2016_Multijet_Pred_v2.root");         suffixHist.push_back("_CD");
  ...
  for(int i=0;i<name1.size();i++){
    ...
    c_cA[i]->cd();    p_top[i]->cd();
    hs_hist[i]->SetMinimum(0.5);
    hs_hist[i]->SetMaximum(100000);
    //hs_hist[i]->Draw("BAR");
    ...
    h_numr->GetYaxis()->SetTitle("#frac{Data}{SM}");
