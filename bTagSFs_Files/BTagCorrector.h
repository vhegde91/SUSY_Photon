...
class BTagCorrector {
	public:
		...
		void SetEffs(TString dirName){
		  TFile *file = TFile::Open("Eff_bTag_Summer16.root");
		  TDirectory *histDirectory = (TDirectory*)file->FindObjectAny(dirName);
			h_eff_b = (TH2F*)histDirectory->Get("h_eff_b");
			h_eff_c = (TH2F*)histDirectory->Get("h_eff_c");
			h_eff_udsg = (TH2F*)histDirectory->Get("h_eff_udsg");
			
			//if these are not found, check for their components
			if(!h_eff_b){
				TH2F* n_eff_b = (TH2F*)histDirectory->Get("n_eff_b");
				TH2F* d_eff_b = (TH2F*)histDirectory->Get("d_eff_b");
				h_eff_b = (TH2F*)n_eff_b->Clone("h_eff_b");
				h_eff_b->Divide(d_eff_b);
			}
			if(!h_eff_c){
				TH2F* n_eff_c = (TH2F*)histDirectory->Get("n_eff_c");
				TH2F* d_eff_c = (TH2F*)histDirectory->Get("d_eff_c");
				h_eff_c = (TH2F*)n_eff_c->Clone("h_eff_c");
				h_eff_c->Divide(d_eff_c);
			}
			if(!h_eff_udsg){
				TH2F* n_eff_udsg = (TH2F*)histDirectory->Get("n_eff_udsg");
				TH2F* d_eff_udsg = (TH2F*)histDirectory->Get("d_eff_udsg");
