...
void LostMuon::EventLoop(const char *data,const char *inputFileList) {
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    /////////////////////////////////////////////////////////////
    cout<<"No. of L1 objects:"<<L1EGObjects->size()<<endl;
    ...
    cout<<"No. of HLT objects:"<<HLTElectronObjects->size()<<endl;
    ...
    cout<<"Trigger:"<<endl;
    bool passEle27Trig = false;
    for(int i=0;i<TriggerNames->size();i++){
      string trgName=(*TriggerNames)[i];
      trgName.pop_back();
      if( trgName=="HLT_Ele27_WPTight_Gsf_v"){
	cout<<trgName<<" : "<<(*TriggerPass)[i]<<endl;
	if((*TriggerPass)[i]) passEle27Trig = true;
      }
    }
    cout<<"No. of Electrons:"<<Electrons->size()<<endl;
    for(int i=0;i<Electrons->size();i++){
      cout<<"ElePt: "<<(*Electrons)[i].Pt()<<" Eta: "<<(*Electrons)[i].Eta()<<" Phi: "<<(*Electrons)[i].Phi()<<" M: "<<(*Electrons)[i].M()<<" TighID: "<<(*Electrons_tightID)[i]<<endl;
    }
    cout<<"No. of photons:"<<Photons->size()<<endl;
    for(int i=0;i<Photons->size();i++){
      cout<<"Pt:"<<(*Photons)[i].Pt()<<" eta:"<<(*Photons)[i].Eta()<<" phi:"<<(*Photons)[i].Phi()<<endl;
    }
    if(passEle27Trig && (HLTElectronObjects->size()==0 || L1EGObjects->size()==0) ) cout<<":(((((((((1111111111(((((((((((((((((("<<endl;
    cout<<endl;
    if(!passEle27Trig) cout<<"TrigNotPassedTrigNotPassedTrigNotPassedTrigNotPassedTrigNotPassedTrigNotPassed"<<endl;
    if(HLTElectronObjects->size()==0) cout<<"HLTHLT    HLT    HLT    HLT    HLT    HLT    HLT    HLT    "<<endl;
    if(L1EGObjects->size()==0) cout<<"L1L1    L1    L1    L1    L1    L1    L1    L1    "<<endl;
