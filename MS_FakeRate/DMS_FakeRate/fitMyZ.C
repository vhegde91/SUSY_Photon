...
void fitMyZ(){
  ...
  TFile *f1 = new TFile("DMS_Run2016_FR_NoZWindow.root");
  TH1D *h_zmass=(TH1D*)f1->FindObjectAny("ZMass_Pho");
  ...
  TF1 *ft1=new TF1("ft1",fitFunction,50,400,6);
  ft1->SetNpx(5000);
  ft1->SetParameter(0,0.5);  // ft1->SetParName(0,"const");  
  ft1->SetParameter(1,0.03); //  ft1->SetParName(2,"mean");  
  ft1->SetParameter(2,-2);  //   ft1->SetParName(1,"sigma");
  // ft1->SetParameter(3,2.0);
  // ft1->SetParameter(4,90.0);
  // ft1->SetParameters(1,1,1,1,1,1);
  // h_zmass->Fit("ft1","0");
  ft1->SetParameter(4,4); // width
  ft1->SetParameter(5,91);   // peak
  h_zmass->Fit("ft1","V+","ep",50,150);

  // improve the picture:
  TF1 *backFcn = new TF1("backFcn",background,50,150,3);
  backFcn->SetLineColor(kGreen);
  TF1 *signalFcn = new TF1("signalFcn",mybw,50,150,3);
