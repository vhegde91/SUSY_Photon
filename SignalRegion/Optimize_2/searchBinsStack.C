...
void searchBinsStack(){
  ...
  for(int i=0;i<nfiles;i++){
    ...
    TH1D *h_MET=(TH1D*)f[i]->FindObjectAny("AllSBins_v5");//MET_R1
    ...
    // for(int j=1;j<=h_MET->GetNbinsX();j++){
    //   cout<<h_MET->GetBinContent(j)<<endl;
    // }cout<<endl;
...
void drawlegend(TH1D *hist,int i,const char* fname){
  ...
  */
  ...
  else if(lName.Contains("bbbbZg_1600_150")){lName="T5bbbbZG_150";}
  else if(lName.Contains("bbbbZg_1600_1000")){lName="T5bbbbZG_1000";}
  else if(lName.Contains("bbbbZg_1600_1550")){lName="T5bbbbZG_1550";}

  //  const char *l_name=lName.c_str();
  TString l_name=lName;//.c_str();
  if(i<5){
