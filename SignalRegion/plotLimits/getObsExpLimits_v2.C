...
double intLumi = 35.9;
void getObsExpLimits_v2(TString fName){
  //---------------- set styles----------------
  gStyle->SetOptStat(0);
  gStyle->SetTitle(0);
  const Int_t NRGBs = 5;
  ...
  if(modelName.Contains("T6tt")                                )  { xMin = 300;  xMax = 2450; nDivAxis = 510;}

  TPaveText *decayMode = new TPaveText(0.15,0.85, 0.8,0.9,"NDC");
  decayMode->SetShadowColor(0);   decayMode->SetFillColor(0); 

  TLegend *legText = new TLegend(0.17,0.73, 0.45,0.84);
  ...
  modelName="T5bbbbZg";}
    ...
    decayMode->AddText("#bf{pp#rightarrow#tilde{t}#tilde{#bar{t}}, #tilde{t}#rightarrow t#tilde{#chi}_{1}^{0}, #tilde{#chi}_{1}^{0}#rightarrow #gamma/Z#tilde{G}  NLO+NLL Exclusion}");
  modelName="T6ttZg";}
