...
void getObsExpLimits(TString fName){
  ...
  double xMin = 1400, xMax = 2450;
  double yMin = 0, yMax = 3100;
  int nDivAxis = 505;
  TString modelName=f1->GetName();
  if(modelName.Contains("T5bbbb") || modelName.Contains("T5qqqq")){ xMin = 1400; xMax = 2450; nDivAxis = 510;}
  if(modelName.Contains("T5tttt") || modelName.Contains("T6tt"))  { xMin = 1400; xMax = 2450; nDivAxis = 510;}

  TPaveText *decayMode = new TPaveText(0.15,0.7, 0.8,0.9,"NDC");
  decayMode->SetShadowColor(0);   decayMode->SetFillColor(0); 

  TLegend *legText = new TLegend(0.17,0.71, 0.51,0.83);
  ...
  //TH2D *h2_temp=new TH2D("h2temp","h2temp",400,0,4000,400,0,4000);
    
  //  h2_XsecUL->SetBinContent(h2_XsecUL->GetXaxis()->FindBin(1400),h2_XsecUL->GetYaxis()->FindBin(1.),10);
  //------------------------------------------------
  ...
  TCanvas *c3=new TCanvas("c3","c3",1500,1100);
  //c3->SetBottomMargin(0.12);
  c3->SetLeftMargin(0.15);
  c3->SetRightMargin(0.2);
  
  // h2_temp->GetXaxis()->SetRangeUser(xMin,xMax);
  // h2_temp->GetYaxis()->SetRangeUser(yMin,yMax);
  // h2_temp->Draw("text");
  int k=0;
  // gStyle->Reset();
  //------------------------------------------------
  ...
  gr2dXsec->GetYaxis()->SetNdivisions(505);
  ...
  // decayMode->AddText("#bf{Observed #pm 1#sigma_{theory}}");
  // decayMode->AddText("#bf{Expected #pm 1#sigma_{experiment}}");
  decayMode->AddText(" ");
  decayMode->AddText(" ");
  //decayMode->SetMargin(0.0);
  decayMode->Draw();

  legText->SetTextSize(0.05);
  ...
  gr2dXsec->GetZaxis()->SetTitle("95% CL upper limit on #sigma(pb)");
