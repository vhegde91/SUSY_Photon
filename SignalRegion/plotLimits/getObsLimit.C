...
double intLumi = 35.9;
void getObsLimit(TString fName){
  ...
  TH2D *h2_rLow=(TH2D*)f1->FindObjectAny("mGlmNLSP_rUnc");
  TH2D *h2_rHigh=(TH2D*)h2_rLow->Clone("mGlmNLSP_rHigh");
  
  h2_rLow->Scale(-1); h2_rLow->Add(h2_rvalue);
  h2_rHigh->Scale(1); h2_rHigh->Add(h2_rvalue);
  ...
  //-----------------------
  TGraph2D *gr2d_rLow = new TGraph2D(h2_rLow);
  TGraph2D *gr2d_rHigh = new TGraph2D(h2_rHigh);  
  TList *listrLow =gr2d_rLow->GetContourList(1.0);
  TList *listrHigh =gr2d_rHigh->GetContourList(1.0);
  TGraph *gr1d_rLow=(TGraph*)listrLow->First();
  TGraph *gr1d_rHigh=(TGraph*)listrHigh->First();
  //----------------------
  int k=0;
  // gStyle->Reset();
  gr1d->SetLineColor(kRed);
  gr1d->SetLineWidth(2);
  gr1d_rLow->SetLineColor(kRed);
  gr1d_rLow->SetLineWidth(2);gr1d_rLow->SetLineStyle(2);
  gr1d_rHigh->SetLineColor(kRed);
  gr1d_rHigh->SetLineWidth(2);gr1d_rHigh->SetLineStyle(2);
