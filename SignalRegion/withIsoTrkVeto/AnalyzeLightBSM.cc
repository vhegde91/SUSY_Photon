...
void AnalyzeLightBSM::EventLoop(const char *data,const char *inputFileList) {
  ...
  string str;
  ifstream ipfile(inputFileList);
  if(!ipfile){cout<<"Couldn't Open File "<<inputFileList<<endl;}
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    if(s_data=="SignalZ") hadJetID = true;
    if(process){
      evtSurvived++;
...
void AnalyzeLightBSM::print(Long64_t jentry){
  ...
  for(int i=0;i<Jets->size();i++){
    if( ((*Jets)[i].Pt() > MHT_PtCut) && (abs((*Jets)[i].Eta()) <= MHT_EtaCut) ){
      cout<<"JetPt:"<<(*Jets)[i].Pt()<<" JetEta:"<<(*Jets)[i].Eta()<<" JetPhi:"<<(*Jets)[i].Phi()<<" QMult:"<<(*Jets_chargedMultiplicity)[i]<<endl;
    }
  }
  int nChargePf_leadJet=0;
  for(int i=0;i<PFCands->size();i++){
    if((*PFCands_charge)[i]!=0 && (*Jets)[0].DeltaR((*PFCands)[i]) < 0.5){ 
      nChargePf_leadJet++;
      cout<<"PF cand Pt:"<<(*PFCands)[i].Pt()<<" eta:"<<(*PFCands)[i].Eta()<<" phi:"<<(*PFCands)[i].Phi()<<" charge:"<<(*PFCands_charge)[i]<<" id:"<<(*PFCands_id)[i]<<" dR with Jet0:"<<(*Jets)[0].DeltaR((*PFCands)[i])<<endl;
    }
  }
  cout<<"nChargePf_leadJet:"<<nChargePf_leadJet<<endl;
