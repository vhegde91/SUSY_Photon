...
const int nfiles=10,nBG=7;    //Specify no. of files
TFile *f[nfiles];
...
void StackHists(){
  ...
  f[3] = new TFile("TTJetsAll.root");
  f[4] = new TFile("QCD.root");
  f[5] = new TFile("GJets.root");
  f[6] = new TFile("WGJetsToLNuG.root");
  f[7] = new TFile("HG_NLSP150.root");
  f[8] = new TFile("HG_NLSP1000.root");
  //f[7] = new TFile("HG_1300_1250.root");
  f[9] = new TFile("HG_NLSP1550.root");
  ...
  for(int i=0;i<nfiles;i++){
    ...
    if(i==nBG-1){
      c_cA->cd(1); hs_ST->          Draw("BAR HIST");hs_ST->          GetXaxis()->SetTitle("ST(GeV)"); decorate(hs_ST,          i,f[i]->GetName()); 
      c_cA->cd(2); hs_MET->         Draw("BAR HIST");hs_MET->         GetXaxis()->SetTitle("MET(GeV)"); decorate(hs_MET,         i,f[i]->GetName()); 
      c_cA->cd(3); hs_nJets->       Draw("BAR HIST");hs_nJets->       GetXaxis()->SetTitle("no. of Jets"); decorate(hs_nJets,       i,f[i]->GetName()); 
      c_cA->cd(4); hs_BTags->       Draw("HIST BAR");hs_BTags->       GetXaxis()->SetTitle("no. of B-Tags"); decorate(hs_BTags,       i,f[i]->GetName()); 
      c_cA->cd(5); hs_BestPhotonPt->Draw("HIST BAR");hs_BestPhotonPt->GetXaxis()->SetTitle("Best Photon Pt(GeV)"); decorate(hs_BestPhotonPt,i,f[i]->GetName()); 

      c_cB->cd(1); hs_STvBin->      Draw("HIST BAR");hs_STvBin->      GetXaxis()->SetTitle("ST(GeV)");              decorate(hs_STvBin,     i,f[i]->GetName());
      c_cB->cd(2); hs_METvBin->     Draw("HIST BAR");hs_METvBin->     GetXaxis()->SetTitle("MET(GeV)");             decorate(hs_METvBin,    i,f[i]->GetName());
      c_cB->cd(3); hs_BestPhotonPtvBin->Draw("HIST BAR");hs_BestPhotonPtvBin->GetXaxis()->SetTitle("Best Photon Pt(GeV)");decorate(hs_BestPhotonPtvBin,    i,f[i]->GetName());
    }
    if(i>=nBG){
...
void decorate(TH1D* hist,int i,const char* fname){
  ...
  if(i<nBG) hist->SetFillColor(col[i]);
...
void drawlegend(TH1D *hist,int i,const char* fname){
  ...
  */
  ...
  else if(lName=="DYJetsToLL"){lName="DY( #rightarrow l^{+}l^{-})+jets";}
  else if(lName=="WJetsToLNu"){lName="W( #rightarrow l  #nu)+jets";}
  else if(lName=="RareProcess"){}
  else if(lName=="TTJetsAll"){lName="t #bar{t}+jets";}
  else if(lName=="QCD"){lName="QCD";}
  else if(lName=="GJets"){lName="G + jets";}
