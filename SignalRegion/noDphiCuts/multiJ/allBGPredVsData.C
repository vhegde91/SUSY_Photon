...
void allBGPredVsData(TString iFname){
  ...
  for(int i=0;i<name1.size();i++){
    ...
    if(compAC){
      h_histG=(TH1D*)f[0]->FindObjectAny("mindPhi1dPhi2_A");
      h_histG->Add((TH1D*)f[0]->FindObjectAny("mindPhi1dPhi2_C"));
    }
    else h_histG=(TH1D*)f[0]->FindObjectAny(name);
    ...
    for(int p=1;p<nfiles;p++){  
      ...
      if(compAC){
	h_histE=(TH1D*)f[p]->FindObjectAny("mindPhi1dPhi2_A");
	h_histE->Add((TH1D*)f[p]->FindObjectAny("mindPhi1dPhi2_C"));
      }
      else h_histE=(TH1D*)f[p]->FindObjectAny(name);
